use crate::util::geometry::Plane;
use crate::util::node::BindingGroupSetting;
...
impl<'a, T: Vertex + AsBytes> ViewNodeBuilder<'a, T> {
    pub fn new(
        ...
        ViewNodeBuilder {
            attributes: NodeAttributes {
                ...
                tex_rect: None,
                corlor_format: None,
                color_blend_state: Some(crate::util::utils::default_blend()),
    ...
    pub fn with_vertex_buffer_layouts(
        mut self, layouts: Vec<wgpu::VertexBufferLayout<'a>>,
    ) -> Self {
    ...
    pub fn with_dynamic_uniforms(
        mut self, uniforms: Vec<(&'a BufferObj, wgpu::ShaderStages)>,
    ) -> Self {
    ...
    pub fn with_tex_views(
        mut self, views: Vec<(&'a AnyTexture, Option<StorageTextureAccess>)>,
    ) -> Self {
...
#[allow(dead_code)]
impl ViewNode {
    fn frome_attributes<T: Vertex + AsBytes>(
        attributes: NodeAttributes<T>, device: &wgpu::Device,
    ) -> Self {
        ...
        let (vertex_buf, index_data) = if let Some(vi) = attributes.vertices_and_indices {
            ...
            let factor = crate::util::utils::matrix_helper::fullscreen_factor(attributes.view_size);
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            depth_stencil: if attributes.use_depth_stencil {
                Some(crate::util::depth_stencil::create_state())
            } else {
                None
            },
        ...
        ViewNode {
            ...
            dy_uniform_bg,
            pipeline,
            clear_color: crate::util::utils::alpha_color(),
        }
    }

    // 视口的宽高发生变化
    pub fn resize(&mut self, queue: &wgpu::Queue, tex_rect: Option<app_surface::math::Rect>) {
    ...
    pub fn draw_by_instance_count<'a, 'b: 'a>(
        &'b self, rpass: &mut wgpu::RenderPass<'b>, instance_count: u32,
    ) {
    ...
    pub fn begin_rpass_by_offset(
        &self, frame_view: &wgpu::TextureView, encoder: &mut wgpu::CommandEncoder,
        load_op: wgpu::LoadOp<wgpu::Color>, offset_index: u32,
    ) {
