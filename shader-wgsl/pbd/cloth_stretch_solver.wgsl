...
fn main([[builtin(global_invocation_id)]] global_invocation_id: vec3<u32>) {  
  ...
  for (var i = 0; i < 8; i = i + 1) {
    ...
    var constraint_obj = constraints.data[constraint_index];
    if (invert_mass0 < 0.0) {
      particle0_index = constraint_obj.particle0;
      particle = particles.data[particle0_index];
      invert_mass0 = particle.uv_mass.z;
    }

    var particle1 = particles.data[constraint_obj.particle1];
    ...
    // Cj(x)
    let constraint = dis - constraint_obj.rest_length;

    var correction_vector: vec4<f32>;
    // eq.18
    let dlambda = -(constraint + cloth.compliance * constraint_obj.lambda) / (sum_mass + cloth.compliance);
    // eq.17
    correction_vector = dlambda * p0_minus_p1 / (dis + EPSILON);

    constraint_obj.lambda = constraint_obj.lambda + dlambda;
    // 更新位置
    ...
    if (is_movable_particle(particle1)) {
      ...
      particles.data[constraint_obj.particle1] = particle1;
    }
    constraints.data[constraint_index] = constraint_obj;
  }
